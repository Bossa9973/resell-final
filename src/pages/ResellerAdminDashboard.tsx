...
export function ResellerAdminDashboard() {
  ...
  useEffect(() => {
    if (resellers.length > 0) {
      ...
      const activeSubscribersTotal = resellers.reduce((sum, r) => sum + (r.clients?.length || 0), 0);
                        ...
                        <div>
                          ...
                          <div className="flex items-center flex-wrap gap-4 text-[10px] text-zinc-400 mt-4">
                            ...
                            <span className="flex items-center gap-1">
                              ...
                              Active orders: <strong className="text-zinc-300">{res.orders?.length || 0} nodes</strong>
                            </span>
                          </div>
                        </div>
              ...
              <div className="space-y-6">
                <div className={`p-6 rounded-3xl border ${theme === 'light' ? 'bg-white border-slate-200 shadow-sm' : 'bg-[#0b0f14] border-white/5'} space-y-4`}>
                  ...
                  <div className="space-y-3.5 pt-2">
                    {/* AUTOFILL LINK FROM REGISTERED USERS DROPDOWN */}
                    <div className="bg-indigo-500/10 border border-indigo-500/20 rounded-2xl p-3.5 space-y-2">
                      <label className="text-[10px] text-indigo-400 font-transparent font-black uppercase font-mono block">🔗 Select Registered User Account</label>
                      <p className="text-[9px] text-zinc-400 leading-normal">
                        Choose any existing client user. This binds their account and auto-fills details instantly.
                      </p>
                      ...
                      <select
                        ...
                        onChange={(e) => {
                          ...
                          setNewResellerId(val);
                          if (val) {
                            const selectedUser = users.find(u => u.id === val);
                            if (selectedUser) {
                              const baseEmail = selectedUser.email || "";
                              const fallbackLabel = baseEmail ? baseEmail.split('@')[0] : "Cloud";
                              const computedBrand = (selectedUser.fullName || fallbackLabel) + " Hosting";
                              
                              setNewResellerName(computedBrand);
                              setNewResellerEmail(baseEmail);
                              showNotification(`Linked & pre-populated for ${baseEmail}!`, "info");
                            }
                          } else {
                            setNewResellerName("");
                            setNewResellerEmail("");
                        ...
                        className={`w-full bg-black/40 border border-white/5 rounded-xl px-3 py-2 text-xs focus:outline-none focus:border-indigo-500 ${
                          theme === 'light' ? 'text-slate-800 bg-slate-100 border-slate-350' : 'text-zinc-200'
                        }`}
                      >
                        <option value="">-- Call to select from registered clients --</option>
                        {users.map(u => (
                          <option key={u.id} value={u.id}>
                            {u.fullName || u.email || "No Name"} ({u.email || u.id})
                          </option>
                        ))}
                      </select>
                    </div>
